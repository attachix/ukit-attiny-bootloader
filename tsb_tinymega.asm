...
;2017 Slavey Karadzhov - Hardware serial
;                       - Compilation automation with a makefile
...
;-----------------------------------------------------------------------
;
; Important Note: A7/B0 are the defaults for making database template
;
.equ    RXPORT  = PORTA
.equ    RXPIN   = PINA
.equ    RXDDR   = DDRA
...
#ifdef DEBUG
.message "AVR-BTL: Debug ON"
#endif


#ifdef HARDWARE_UART

.message "AVR-BTL: Using Hardware UART"
...
; RxD_from_WF    =PA7 - Input  - from RS232 COM Port /RxD0 - za premane ot WF modul (USART0)/ (Internaly pulled high)
; TxD_to_WF      =PB0 - Output - to RS232 COM Port /TxD0 - za predavane kym WF modul (USART0)/
.SET POWER_WF_EN    =PA6 ; Output - power WF module enable  (Active = 1)
.SET RESET_WF       =PA2 ; Output - resets the WF module

.EQU    PUEAini =0B11000000
                ;PA7-PullUp, PA6-PullUp;
.EQU    PUEBini =0B00000001
                ;PB0-PullUp
...
actw2:
        ...
        brne actw1
actwx:
        ldi xl, 160 ; =0xa0 Hardcoded value for 9600 baude rate
        movw bclkl, xl                  ; save result to bcl

#else
;==============================================================================
; Activate    -> Wait for receive confirm byte "@" from WF module
; Input: ACCA - Timeout for waiting [sec]
; Otput: ACCD --> =0 - OK ; =1 - ERROR (timout)
; Destroyed registers: ACCA, ACCD
;==============================================================================
Activate:
    SBI PORTA, POWER_WF_EN

#ifdef ENABLE_FACTORY_RESET

.message "AVR-BTL: Factory Reset is enabled."

CheckReset:
    in tmp4, MCUSR                  ; check reset condition
    sbrs tmp4, EXTRF                ; if this is not an external reset
    rjmp actw100                    ; jump to activation

#ifdef DEBUG_RESET
    CBI PORTB,PB3 ; LED on
    ldi ACCA, 'R'
    rcall TransmitByte
#endif
    ...
    CBI PORTA, RESET_WF             ; RESET WF

                                    ; wait 10ms at 1.0 MHz
    ldi  r18, 13
    ldi  r19, 252
...
CheckReset_L1:
    ...
    SBI PORTA, RESET_WF             ; clear reset

                                    ; wait for MCU2 to boot: 100ms at 1.0 MHz
    ldi  r18, 130
    ldi  r19, 222
CheckReset_L2:
...
#ifdef DEBUG_RESET
    SBI PORTB,PB3 ; LED off
#endif

#endif

actw100:
    ; Wait for first confirmation byte @
    ; If timeout or error on reading -> go to APPJUMP
    rcall   ReceiveByteUSART0   ; Receive Byte from USART0
                ; Check for timeout or ERR
    tst ACCD
    brne    actwAppJump     ; go to end if timeout or ERR

    cpi ACCA,Confirm_byte   ;proverka za poluchen bajt za potvyrjdenie
    brne    actw100         ; go to receive new byte

actw101:
    ; Wait for the second confirmation byte @
    ; If timeout or error on reading -> to to APPJUMP
    ...
    cpi ACCA,Confirm_byte   ;proverka za poluchen bajt za potvyrjdenie
    brne    actwAppJump     ; go to end if byte is not as expected

actw102:
    ; Wait for the third, last, confirmation byte @
    rcall   ReceiveByteUSART0   ; Receive Byte from USART0
                ; Check for timeout or ERR
    tst ACCD
    brne    actwAppJump     ; go to end if timeout or ERR
    ...
    brne    actwAppJump     ; go to end if byte is not as expected
                ...
                ; Received all 3 confirmation bytes "@@@" -> we can start the bootloader
    ldi ACCD,0  ;return code = OK
    rjmp actw_end

actwAppJump:
    CBI PORTA,POWER_WF_EN
...
; Destroyed registers: ACCA, ACCD, X
; Fixed timeout: circa 4 seconds
;==============================================================================
ReceiveByteUSART0:
    push    ACCC
    push    ACCB
...
wbc1:
        sbiw xl, 24                     ; nearly same number of clocks
